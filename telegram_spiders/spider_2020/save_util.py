...
import pytz
from telethon.tl.types import UserStatusOnline, UserStatusOffline, UserProfilePhoto, Message, PeerUser, PeerChannel, \
    PeerChat, Channel, User, MessageMediaDocument
...
def delete_session_file(phone):
    os.remove(sessions_dir + str(phone) + "_session.txt")
        ...
        if isinstance(message, Message):
            if message.media:
                ...
                if isinstance(entity, Channel):
                    media_dir = 'channel_messages/channel_id_' + str(message.to_id.channel_id) + '/'
                elif isinstance(entity, User):
                    media_dir = 'user_messages/user_id_' + str(message.to_id.user_id) + '/'
                else:
                    media_dir = 'group_messages/chat_id_' + str(message.to_id.chat_id) + '/'
                if not os.path.exists(media_dir):
                    os.mkdir(media_dir)
                path = await message.download_media(media_dir)
                print(path)
